...
def run_model(
    ...
    model = create_model(
        model_type=model_type, task_type=task_type, model_params=model_params
    )
    ...
    if save_to_drive:
        if legacy_save:
            if model_type == "lgb":
                model.save_model(
                    os.path.join(save_folder, f"model_{cv_count}.{model_type}")
                )
            else:
                model.save_model(
                    save_folder + "model_{}.".format(cv_count) + model_type
                )
        else:
            if model_type == "lgb":
                model_json = model.dump_model()
                with open(
                    os.path.join(save_folder, f"model_{cv_count}.{model_type}"), "w"
                ) as f:
