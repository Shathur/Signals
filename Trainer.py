...
class Trainer:
    ...
    def get_more_features(self):
        self.feature_names = [f for f in self.df.columns if (('lag' in f) or ('diff' in f))]
    ...
    def get_vol_features(self):
        ...
        self.feature_names = [f for f in self.df.columns if f not in target_features+vol_non_features]
    ...
    def get_combine_vol_features(self,features_filepath):
        ...
        more = pd.read_parquet(os.path.join(os.path.dirname(__file__),os.pardir,'Data','Signals','more_full_features.parquet'))
        ...
        standard_columns = ['data_type', 'date', 'friday_date', 'ticker', 'bloomberg_ticker']
    ...
    def test_dates(self):
        if ((self.num_tour_weeks>1) and (self.num_tour_weeks%1==0)):
            tour_period = [int((datetime.now() + relativedelta(weekday=FR(-int(i)))).strftime('%Y%m%d')) for i in np.arange(1, self.num_tour_weeks)] 
        else:
    ...
    def get_train_val_test(self, tour_period):
        ...
        self.df = self.df[~self.df['friday_date'].isin(self.tour_df['friday_date'].unique())]
    ...
    def train(self, save_to_drive, save_folder):
        ...
        for count, cv_split in enumerate(cv_split_data):
            ...
            print(f"unique validation dates: {self.df.iloc[cv_split[1]]['friday_date'].nunique()}")
