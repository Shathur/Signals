    ...
    for i in range(_iterations):
        ...
        if i > 0:
            for era in era_scores_tour.index:
                ...
                dic = {'iteration': 'it' + str(i), date_col: '_' + era[3:], 'correlation_changes': era_diff_tour[era]}
        ...
        worst_eras_df = train_df[train_df[date_col].isin(worst_eras)]
...
def era_boost_val_criterion(
    ...
    """
    function that trains iteratively improving on the weakest eras.

    _trees_per_step -- the number of trees added in each iteration
    _era_quantile -- is the percentage of bad eras to train on
    stop_criterion -- number of iterations
    ...
    while loop_condition:
        ...
        if iter_count > 0:
            for era in era_scores_tour.index:
                era_diff_tour[era] = era_scores_tour[era] - era_scores_tour_prior[era]
                dic = {'iteration': 'it' + str(iter_count), date_col: '_' + era[3:], 'correlation_changes': era_diff_tour[era]}
                era_diff_tour_total_lst.append(dic)
            # plt.figure()
            # sns.barplot(x=era_diff_tour.index.str.slice(3), y=era_diff_tour)
            # plt.show()
