...
def run_analytics(era_scores, plot_figures=False):
    if plot_figures:
        ...
        print("\n")
    try:
        hit_rate = era_scores.apply(lambda x: np.sign(x)).value_counts()[1] / len(
            era_scores
        )
...
def features_targets_correlations(df, feature_cols, time_col, target, visualize=False):
    ...
    feature_corrs = df.groupby(time_col).apply(
        lambda x: x[feature_cols].corrwith(x[target])
    )
    ...
    if visualize:
        ...
        sns.barplot(
            x="index", y="avg", data=average_feature_corrs_df.reset_index(), ax=ax
        )
...
def check_if_live(sub, date_col):
